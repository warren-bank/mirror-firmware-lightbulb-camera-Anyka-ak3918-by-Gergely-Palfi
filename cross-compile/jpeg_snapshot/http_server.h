...
void web(int fd, int hit)
{
  ...
  if(!strcmp(fstr, "image/jpeg")){
    ...
    snapshot_http->count = 25; //request to start encoding a few images (encoder will only stop when no more requests are made)

    logger(LOG,"SEND",&buffer[5],hit);

    len = snapshot_http->jpeg[!snapshot_http->capture].len;
    (void)sprintf(buffer,"HTTP/1.1 200 OK\nServer: nweb/%d.0\nContent-Length: %ld\nConnection: close\nContent-Type: %s\n\n", VERSION, len, fstr); /* Header + a blank line */

    logger(LOG,"Header",buffer,hit);
    (void)write(fd,buffer,strlen(buffer));

    (void)write(fd,snapshot_http->jpeg[!snapshot_http->capture].data,len);
...
int start_server(int port, char *webdir, snapshot_t *snapshot){  //webdir not used for anything now because content is served from memory, but can be for other files
	int i, pid;
