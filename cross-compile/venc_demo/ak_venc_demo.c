...
#define BACK_ISP_PATH 		"/usr/local/"		//sub config path
...
static int venc_set_resolution(void *handle)
{
	...
	attr.res[VIDEO_CHN_MAIN].max_width = resolutions[sub_index].width;
	attr.res[VIDEO_CHN_MAIN].max_height = resolutions[sub_index].height;
...
/* init video input */
/*static void *venc_init_video_in()
{
...
static int venc_init_video_input(const char *first, const char *second)
{
	...
	/* set capture resolution */
	if (venc_set_resolution(video_in_handle)) {
		ak_print_error_ex("\tset resolution failed\n");
	...
	/* set capture fps */
	ak_vi_set_fps(video_in_handle, 25);
	ak_print_normal("\tcapture fps: %d\n", ak_vi_get_fps(video_in_handle));
...
static int venc_start_stream(enum encode_group_type grp)
{
	...
	ak_vi_set_flip_mirror(video_in_handle, 1, 1);
